...
{
    int ret = 0;
    ret = hal_media_get_framebufferInfo(&fbp, &vinfo, &finfo);
    if(ret == 0)
    {
        printf("hal_media_get_framebufferInfo ok! fbp:%x==%d %d====\n", (int)fbp, vinfo.xres, vinfo.yres);
    }else{
        printf("hal_media_get_framebufferInfo,failure!\n");
    }
}
